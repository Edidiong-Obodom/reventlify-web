...
export const SidebarItem = ({ label, icon, badge, isMenuOpen, isMobile }: SidebarItemProps) => {
  return (
    <button
    ...
    >
      ...
      <span
        className={`ml-3 transition-opacity duration-300 ${!isMobile && !isMenuOpen ? "opacity-0 w-0" : "opacity-100"}`}
      >
        {label}
      </span>
      {badge && (
        <span
          className={`${
            !isMobile && !isMenuOpen ? "absolute -top-1 -right-1 w-5 h-5 flex items-center justify-center" : "ml-auto"
